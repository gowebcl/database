drop function if exists batch_indicators_set_dollar;

create or replace function batch_indicators_set_dollar (
...
declare
    ...
    var_obj_series json;
    var_txt_function text = 'batch_indicators_set_dollar';
    var_rec_day record;
...
begin
    ...
    if (var_num_year = var_num_actual) then
        ...
        loop
            ...
            from
                srv_dollar_values
            where
                idf_dollar_value = var_rec_day.num_previous;
    ...
    for var_rec_day in
        select
            idf_dollar_value,
            to_char (to_date (idf_dollar_value::text, 'yyyymmdd') - '1 day'::interval, 'yyyymmdd')::numeric "num_previous",
            num_value
        from
            srv_dollar_values
        where
    ...
    loop
        ...
        where
            idf_dollar_value = var_rec_day.idf_dollar_value;
